...
public class Dealer {
    ...
    //ゲームの準備をするメソッド
    public void prepareGame(ArrayList<Player> player, Deck Deck, Table field) {
        setField(field);
        setPlayer(player);
        setDeck(Deck);
        handOutCards();
    ...
    public void handOutCards() {
        ...
        //山札をシャッフルする
        Collections.shuffle(getDeck().getDeck());
        //山札がなくなるまで１枚ずつカードを配る
        while (getDeck().getDeck().isEmpty() == false) {
            for (int i = 0; i < getPlayer().size(); i++) {
                if (getDeck().getDeck().isEmpty()) {
                    break;
                }
                getPlayer().get(i).setHand(getDeck().getDeck().get(0));
